# INTERNAL: Context Window Manager — Planning & Strategy

> **This file is for internal planning.** The external-facing README.md is what
> maintainers see. This file preserves session context, design source attribution,
> and strategy notes.

Session history: `58f6803b` (2026-02-28), continuing from `8e491163` (2026-02-28).

## Design Sources (Internal Attribution)

| Internal Project | Pattern | Public Description |
|-----------------|---------|-------------------|
| ckd (ck-dolt) | Hexagonal architecture, ports & adapters, interface probing | "Hexagonal architecture pattern" |
| ACF Traces | Bronze/Silver/Gold medallion, hook-driven capture | "Medallion data lifecycle pattern" |
| ACF Beads | Per-project Dolt databases, launchctl management (ADR-2007) | "Prior Dolt deployment experience" |
| Claude Code | Lifecycle hooks with rich payloads | "Claude Code hook model" (public) |
...
1. **Ship progressive disclosure first** (no new deps, immediate value, earns credibility)
2. **Open GitHub Discussion** for storage abstraction (ask before building)
3. **StoragePort PR** if team agrees (pure refactor, feature-flagged)
4. **Dolt adapter** behind `KILO_EXPERIMENTAL_DOLT` (opt-in only)
...
|----------|--------|-----------|
| Backend abstraction | Ports & Adapters (hexagonal) | App layer never imports adapters. Swap via composition root |
...
| Data lifecycle | Bronze/Silver/Gold medallion | Medallion data lifecycle, proven in production |
...
- [Dolt documentation](https://docs.dolthub.com/) — versioned SQL database
